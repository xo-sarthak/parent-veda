...
class ProgramDetailScreen extends StatelessWidget {
  ...
  void _play(BuildContext context, String title, {String? subtitle, String? blurb}) =>
      Navigator.of(context).push(MaterialPageRoute<void>(
          builder: (_) =>
              PrepareVideoScreen(lang: lang, title: title, subtitle: subtitle, blurb: blurb)));
  ...
  void _onCta(BuildContext context, PrepCta cta) {
    ...
    if (!cta.enabled) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(s.uiCohortHasAlreadyStarted),
    ...
    if (cta.watch) {
      if (p.lessons.isNotEmpty) {
        ...
        _play(context, l.title.now, subtitle: s.prepMinutes(l.minutes), blurb: p.subtitle.now);
      } else {
    ...
    showPrepareBooking(
      ...
      heading: p.isLive ? s.prepReserveYourSpot : s.prepAddToYourLibrary,
      cta: cta.label.now,
    );
  }
  ...
  //
  // The VALUES are data or numbers; only the little grey captions under them
  // are chrome, which is why the whole tuple list takes `S` rather than each
  // caller translating at the render site.
  List<(String, String)> _facts(S s) {
    final p = program;
    switch (p.kind) {
      case PrepKind.cohort:
        return [
          (p.durationLabel.now.split(' ·').first, s.prepFactProgramme),
          (s.prepLive, s.prepPlusPeerGroup),
          (p.seatsLeft != null ? '${p.seatsLeft}' : '20', s.prepFactSeatsLeft),
        ];
      case PrepKind.masterclass:
        if (p.isLiveScheduled) {
          return [
            (p.durationLabel.now, s.prepFactLiveLower),
            (s.prepLiveQa, s.prepIncludedLower),
            (s.prepForever, s.prepRecording)
          ];
        }
        return [
          (p.durationLabel.now, s.prepRecorded),
          ('★ ${p.rating}', s.prepRated),
          (s.prepForever, s.prepYours)
        ];
      case PrepKind.course:
        if (p.lessons.isNotEmpty) {
          final mins = p.lessons.fold<int>(0, (t, l) => t + l.minutes);
          return [
            ('${p.lessons.length}', s.prepLessonsLabel),
            ('~$mins', s.prepMinutesLabel),
            ('∞', s.prepLifetime)
          ];
        }
        return [
          (p.durationLabel.now, s.prepCourseLabel),
          ('★ ${p.rating}', s.prepRated),
          ('∞', s.prepLifetime)
        ];
    }
      ...
      body: SafeArea(
        ...
        child: Stack(children: [
          ListView(
            ...
            children: [
              _pad(pvTopBar(context, lang: lang, backLabel: s.uiCoursesCohorts)),
              ...
              _pad(ClipRRect(
                ...
                child: GestureDetector(
                  onTap: () => _play(context, s.prepPreviewOf('${p.title}'),
              ...
              if (p.lessons.isNotEmpty) ...[
                _divider(),
                _pad(Text(s.uiWhatLlLearn, style: pvTitleStyle(17))),
                const SizedBox(height: 4),
                _pad(Text(s.uiShortLessonsOrderStart, style: pvBody(kMuted, 12.5))),
                const SizedBox(height: 12),
                _pad(Column(children: [
                  for (int i = 0; i < p.lessons.length; i++)
                    _lessonRow(context, s, p.lessons[i], accent: a, first: i == 0),
                ])),
              ],
              ...
              if (p.isLive && (p.sessions.isNotEmpty || p.sessionTimes.isNotEmpty)) ...[
                _divider(),
                _pad(Text(p.isCohort ? s.prepYourSchedule : s.prepWhenItRuns, style: pvTitleStyle(18))),
              ...
              if (p.covers.isNotEmpty) ...[
                _divider(),
                _pad(Text(s.uiWhatCovers, style: pvTitleStyle(18))),
              ...
              const SizedBox(height: 20),
              _pad(Text(
                  s.prepLedByVerifiedExpert(
                      '${p.priceNote.now[0].toUpperCase()}${p.priceNote.now.substring(1)}'),
                  textAlign: TextAlign.center, style: pvBody(kMuted, 12).copyWith(height: 1.55))),
            ],
          ),

          Positioned(left: 0, right: 0, bottom: 0, child: _ctaBar(context, s, cta)),
        ]),
      ),
    );
  }
  ...
  Widget _lessonRow(BuildContext context, S s, PrepLesson l,
          {bool first = false, required Color accent}) =>
      GestureDetector(
        onTap: () => _play(context, l.title.now, subtitle: s.prepMinutes(l.minutes)),
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(
                    l.locked
                        ? s.prepMinutesUnlocksLater(l.minutes)
                        : s.prepMinutes(l.minutes),
  ...
  Widget _ctaBar(BuildContext context, S s, PrepCta cta) => AnimatedBuilder(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Row(children: [
                Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text(cta.watch ? s.prepIncluded : program.price,
                      style: pvBody(kInk, 16).copyWith(fontWeight: FontWeight.w700)),
                  Text(cta.watch ? s.prepOnParentVedaPlus : program.priceNote.now,
