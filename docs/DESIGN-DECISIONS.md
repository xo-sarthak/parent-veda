...
| **W06** | ~~**Re-temper the neutrals from lavender to warm paper.**~~ **SUPERSEDED by W06a.** | The neutrals (`#FBF9FE`, `#F3EEF7`, `#ECE5F2`) are cool purple-greys — the same material as iMumz's wash. Same violet on *warm* ground reads completely differently at identical hue. Highest-leverage move available, costs no new colour. | 2026-08-12 |
| **W06a** | **Neutrals desaturated, not re-tempered. `ground` → `#F5F3F6`, `surfaceAlt` → `#EDEAF0` (S36/S33 → S16). Hue and lightness held.** | W06 diagnosed the right problem and prescribed a hue change; the actual fault was **saturation**. Our inks already sat at S7–12 — a correct whisper of brand — while the surfaces sat at S33–36 and read as lilac. Moving to warm cream would have swapped one tint for another *and* walked into the generic AI-design default (cream + serif + terracotta). Dropping saturation to the ink family fixes the iMumz-wash resemblance without spending a colour decision. Lightness deliberately unmoved: elevation here is a line, not a blur, so the `surface`↔`ground` gap is the only thing making a card a card. Applies to `_baseline` (V3) only; the other three token systems follow in the §7 migration. | 2026-08-16 |
