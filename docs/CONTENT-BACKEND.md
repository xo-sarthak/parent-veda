...
- 2026-07-27: **Authors become a record instead of a matching string** (`0053`). The website enriched a post's plain-text `author` by matching it against a hardcoded 139-line array in `src/lib/authors.ts` — so adding an author was a deploy, correcting a credential was a deploy, and typing "Dr Mahender Singh" without the full stop meant the profile silently didn't resolve and the byline went bare. `content_authors` + a real `content_posts.author_slug` FK fixes that; the free-text `author` column stays for posts bylined "Team ParentVeda", which legitimately have no profile. This matters more than a byline here: **articles are AI-drafted and credited to a reviewing clinician**, so the row is the claim that a named, registered doctor stood behind health content — hence the optional `partner_id` link to `care_partners` (trust chain, not two records sharing a name) and `byline_consent_at`. Note `registration` is deliberately PUBLIC — a council number is what makes "Dr." verifiable — and must not be confused with `care_partner_verification` (0050), which is private paperwork. Website half is a handoff, spelled out at the bottom of the migration; the `/guides/authors/[slug]` page must flip `dynamicParams` to true or a newly added author 404s until the next deploy.
- 2026-07-27: **The last unaudited admin act closed** (`0052`). `create_care_partner` predates the audit table, so the log could answer "who approved this doctor" but not "who added them" — the wrong way round, since adding is the moment someone enters the system at all. Dropped and recreated with an actor parameter rather than overloaded: two functions differing only by a defaulted trailing argument make every short call ambiguous, and it would fail at the moment someone is trying to add a doctor.
...
| `0052` | `audit_partner_creation.sql` — the one unaudited admin act | 5 |
| `0053` | `content_authors.sql` — author profiles + the `author_slug` relation | 4 |
| `0054` | `programmes.sql` — one-to-many, Group A (create + sell) | 6 |
